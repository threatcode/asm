...
def init_checks(master_switch, outpath):
    global args
    # Argument Parser
    parser = argparse.ArgumentParser(description='|<------ Attack Surface Mapper - Help Page ------>|',
    ...
    if not (args.target or args.targets):
        ...
        cprint("info", "Example Usage: python3 asm.py -t threatcode.com", 1)
        exit()
...
def sig_handler(signal, frame):
    cprint("info", "\n\n[i] Shutting down Attack Surface Mapper. . .\n", 1)  # Success Msg
    try:
        signal.pause()
    except:
        pass
...
if __name__ == "__main__":
    ...
    cprint("info", "\n\n[i] Attack Surface Mapper is running. . .\n", 1)  # Success Msg
